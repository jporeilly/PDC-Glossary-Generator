...
-- ==========================================
-- Term-resolution columns
-- Added so five Business Glossary terms resolve to real columns:
--   Meter ID          -> monthly_usage.meter_id
--   Meter Reading     -> monthly_usage.meter_reading
--   Tiered Rate       -> tiered_rates.tier_rate
--   System Capacity   -> water_systems.capacity
--   Contaminant Level -> water_quality_reports.contaminant_level
-- After loading, re-run Metadata Ingest + Data Profiling in PDC on the
-- four affected tables so the columns are catalogued and the terms link.
-- ==========================================

-- 1) Meter ID (Low / Public)
ALTER TABLE monthly_usage
    ADD COLUMN IF NOT EXISTS meter_id VARCHAR(30);

COMMENT ON COLUMN monthly_usage.meter_id IS 'Service meter identifier for the customer connection (format MTR-NNNNNN). Stable across billing months.';

UPDATE monthly_usage
    SET meter_id = 'MTR-' || LPAD(customer_id::text, 6, '0');

-- 2) Meter Reading (Low / Public)
ALTER TABLE monthly_usage
    ADD COLUMN IF NOT EXISTS meter_reading INT;

COMMENT ON COLUMN monthly_usage.meter_reading IS 'Cumulative meter register reading in gallons at the end of the billing month. Month usage = current reading minus prior reading.';

UPDATE monthly_usage mu
    SET meter_reading = sub.reading
    FROM (
        SELECT usage_id,
               (100000 + SUM(usage_gallons) OVER (
                   PARTITION BY customer_id
                   ORDER BY billing_month
                   ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW))::INT AS reading
        FROM monthly_usage
    ) sub
    WHERE mu.usage_id = sub.usage_id;

-- 3) Tiered Rate (Medium / Company Confidential)
ALTER TABLE tiered_rates
    ADD COLUMN IF NOT EXISTS tier_rate DECIMAL(8,4);

COMMENT ON COLUMN tiered_rates.tier_rate IS 'Representative published tiered rate (Tier 1 lifeline rate per 1,000 gallons). The full ladder remains in tier1..tier4_rate_per_1000gal.';

UPDATE tiered_rates
    SET tier_rate = tier1_rate_per_1000gal;

-- 4) System Capacity (Low / Public)
ALTER TABLE water_systems
    ADD COLUMN IF NOT EXISTS capacity DECIMAL(10,2);

COMMENT ON COLUMN water_systems.capacity IS 'Rated/permitted annual system capacity in acre-feet. Compare against total_annual_usage_acre_feet for utilization.';

UPDATE water_systems
    SET capacity = ROUND(total_annual_usage_acre_feet * 1.35, 2)
    WHERE total_annual_usage_acre_feet IS NOT NULL;

-- 5) Contaminant Level (Medium / Company Confidential)
ALTER TABLE water_quality_reports
    ADD COLUMN IF NOT EXISTS contaminant_level VARCHAR(20);

COMMENT ON COLUMN water_quality_reports.contaminant_level IS 'Summary contaminant level derived from the regulated readings and compliance status: Low, Moderate, Elevated, High.';

UPDATE water_quality_reports
    SET contaminant_level = CASE
        WHEN compliance_status = 'Violation'     THEN 'High'
        WHEN compliance_status = 'Warning'       THEN 'Elevated'
        WHEN quality_rating   IN ('Fair','Poor') THEN 'Moderate'
        ELSE 'Low'
    END;

-- ==========================================
-- Term-resolution columns complete
-- ==========================================
