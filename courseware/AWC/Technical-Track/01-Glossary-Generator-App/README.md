...
**App version:** 1.6.4 · validated against **PDC 10.2.11**
...
a client through a *domain pack* plus a company name. For the Arizona Water Company
(AWC) scenario it is configured with the bundled `water_utility` pack
(`GLOSSARY_DOMAIN_PACK=domain_packs/water_utility.example.json`) and
`GLOSSARY_COMPANY="Arizona Water Company"`, which reproduce AWC's curated terms
(Customer Record, Water System Record, Water Quality, …). Point it at a different
client by swapping the pack — no code changes.
...
- **Bundled AWC pack** gives every table term a `Record` suffix (*Customer Record*,
  *Rate Plan Record*, …).
...
> The authoritative contents list for this 1.6.4 asset set is **`MANIFEST.md`**.

- **`classification/`** — the **Policy Generator** engine (Python package, v1.6.4):
  Registry, rules-first classifier + sensitivity floor, method emit, drift + reconcile,
  persistence, domain packs, CLI, and a 38-check self-test.
- **`Glossary-Generator-1.6.4.pptx`** — the session deck (original slides + a five-slide
  architecture reference).
- **`Workshop-Glossary-Generator-1.6.4.docx`** — the standalone guide: opens with the
  challenge/goal and the two-apps demarcation, then the workflow, then the architecture section.
- **`CHALLENGE-AND-GOAL.md`** — one-page steward / analyst explainer.
- **`diagrams/`** — the architecture figures (PNG + editable SVG).
- **`awc-datasources.csv`** — the two AWC Workshop 1 connections (PostgreSQL + MinIO),
  pre-filled for the bulk connection loader (`/api/pdc/bulk-load`).
- **`INSTALL.md`** — install the Flask Glossary Generator app against your own PDC instance.
- **`CHANGELOG.md`** — release notes; **1.6.4** is the current version.

The **Flask Glossary Generator app** itself (`glossary_app_v1_5_7.zip`, VERSION `1.5.7`)
is a separate download — it is the app that `INSTALL.md` stands up, and it is not
regenerated in this set.

## Install it on your own PDC instance

If the app isn't already pre-installed on your lab VM — or you're standing it up
against a live AWC PDC instance — follow **`INSTALL.md`**. In short: run it with
Docker (`docker compose up --build`) or locally (`./run.sh`), set
`GLOSSARY_COMPANY` and `GLOSSARY_DOMAIN_PACK` for AWC, smoke-test `GET /health`, then
...
- **Verified compliance references.** Domain packs carry a curated, human-verified
  reference map; the water example ships real EPA and Arizona ADEQ links.
...
![Registry spine](diagrams/registry-spine.png)

![Lifecycle and reconcile](diagrams/lifecycle-loop.png)

![Safe LLM enrichment](diagrams/llm-safety-split.png)

![Layered registry](diagrams/layered-registry.png)
...
(concepts + tags + categories + detection rules). A `water_utility` example pack
ships as **one illustration**; copy it and swap the vocabulary for healthcare,
finance, retail, or any sector without touching code.
