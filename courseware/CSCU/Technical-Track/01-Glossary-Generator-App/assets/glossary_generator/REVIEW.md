...
the Arizona Water (AWC) bundle, roster persistence, module health, regression tests,
...
## 2. Arizona Water (AWC) bundle — applied?
...
What *is* AWC out of the box: `people.json` (the roster — azwater.gov accounts, David
Chen / Maria Garcia / Robert Hayes / Susan Park, with `expertise` seeded).

To apply the AWC vocabulary bundle, set in `.env`:

    GLOSSARY_DOMAIN_PACK=domain_packs/water_utility.example.json

Verified: with that set, `customers -> Customer`, `tiered_rates -> "Rate Plan"`, etc.
...
- **New: `.env` support** — dependency-free loader in `app.py`, runs before local
  imports so `GLOSSARY_DOMAIN_PACK` (AWC bundle), `PORT`, `OLLAMA_URL` etc. all apply.
  See `.env.example`.
...
## 6. Install
    ...
    cp .env.example .env        # optional; uncomment AWC pack / set OLLAMA_URL etc.
