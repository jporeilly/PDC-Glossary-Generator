...
function LabStoreCard() {
  ...
  const epWarn = /:9001(\/|$)/.test(ep) ? 'That’s the web console port — the S3 API is on :9000.'
    : (ep && !/:\d+/.test(ep)) ? 'No port given — the S3 API is on :9000 (e.g. http://pdc.example.com:9000).'
    : ''
  ...
  return (
    <section className="card span2">
      ...
      <div className="form-grid">
        <label>
          Endpoint
          <input type="text" placeholder="http://pdc.example.com:9000" value={cfg.endpoint}
