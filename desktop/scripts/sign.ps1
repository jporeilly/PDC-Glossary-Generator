...
.DESCRIPTION
    ...
    The certificate is never in this repo. It is read from the environment:
        ...
        PDCG_SIGN_THUMBPRINT   SHA-1 thumbprint of a cert in the Windows store
        PDCG_SIGN_TIMESTAMP    RFC-3161 timestamp URL (optional, has a default)
...
$thumb = $env:PDCG_SIGN_THUMBPRINT
if (-not $thumb) {
    Write-Host "  [--] not signing $(Split-Path -Leaf $Path) - PDCG_SIGN_THUMBPRINT is not set"
    exit 0
}

$timestamp = $env:PDCG_SIGN_TIMESTAMP
...
if (-not $signtool) {
    ...
    if (-not $found) {
        throw "PDCG_SIGN_THUMBPRINT is set but signtool.exe was not found - install the Windows SDK"
    }
