...
function tdRenderFacet(){
  ...
  if(empties.length){
    ...
    // freshly (re)seeded dictionaries haven't grown from a scan yet — every
    // count is zero by definition, and on 1.8.25 a retire is DURABLE, so
    // offering the bulk button here would invite gutting the curated pack
    const grown=!!(TAGDICT.sources&&TAGDICT.sources.length);
    const retire=companyEmpties.length?(grown?` <button class="ghost sm" style="padding:1px 8px" onclick="tdRetireEmpty(${JSON.stringify(companyEmpties).replace(/"/g,'&quot;')})" title="Remove the ${companyEmpties.length} empty COMPANY-layer tag(s) from the vocabulary. Durable (tombstoned across reseeds); Export domain pack will offer to remove them from the pack. The generic baseline can't be removed; a tag a rule still emits is re-added with a warning.">Retire ${companyEmpties.length} empty company tag${companyEmpties.length>1?'s':''}</button>`:' <span class="hint">(bulk retire appears after the dictionary has grown from a scan — freshly reseeded counters are all zero and prove nothing)</span>'):'';
