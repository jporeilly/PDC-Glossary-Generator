...
> **Harvest from PDC** picker that scales to 100+ sources; the water-utility domain
> pack; and non-destructive **Enrich** with **↶ Revert enrich**. See `CHANGELOG.md`
> and `courseware/Workshop-Glossary-Generator-1_6_20.docx` (and the matching deck,
> `courseware/Glossary-Generator-1_6_20.pptx`).
>
...
covers common patterns (billing, usage/metering, water-quality, contact, location, …);
...
The shipped `domain_pack.json` is the AWC (water utility) pack — it adds GIS, SCADA,
inspections, correspondence, hydrology, maintenance, and more. See
`domain_packs/README.md` for the full pack schema (categorization keys *and* tag keys),
and `domain_packs/water_utility.example.json` for the complete AWC example.
