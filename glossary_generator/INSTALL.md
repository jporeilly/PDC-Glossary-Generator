...
*App version **1.6.20** · validated against **Pentaho Data Catalog 10.2.11**.*

This guide stands the **Glossary Generator** app up against *your* Pentaho Data
Catalog (PDC) instance — your data sources, your accounts, your network. It targets
the **generic build** of the app (the one with the `domain_packs/` folder, the
`GLOSSARY_*` environment variables, and the `Dockerfile`/`docker-compose.yml`), and
uses **Arizona Water Company (AWC)** as the worked example throughout.

If you are running the original AWC-only build (no `domain_packs/` folder), the
Arizona Water vocabulary is already baked in — skip the *domain pack* step and the
`GLOSSARY_*` variables, and follow everything else as written.
...
PostgreSQL on 5432), and any object store you'll scan (MinIO/S3 endpoint). For AWC's
...
- **`water_utility.people.json`** — a companion **people/steward roster** for the AWC
...
- **`CLASSIFICATION_DOMAIN_PACK`** — path to an optional JSON that overlays
  **industry classification concepts** (term + tags + sensitivity + category +
  ...
  registry that Policy Generator and the drift linter share. Ships a
  `water_utility` example; copy and swap for any sector — no code changes.

For **AWC**, point at the bundled example pack, which reproduces the original Arizona
Water vocabulary (Customer Record, Water System Record, Water Quality, the `ws`/`wtr`
abbreviations, …):

```
GLOSSARY_COMPANY="Arizona Water Company"
GLOSSARY_DOMAIN_PACK=domain_packs/water_utility.example.json
```

For a different customer, copy that file, edit the vocabulary, and point at your copy
(or drop a file named `domain_pack.json` beside `suggester.py`). See
`domain_packs/README.md` for the key reference.
...
2. **Set your scenario values.** Edit `docker-compose.yml` — uncomment and set
   `GLOSSARY_COMPANY`, and add `GLOSSARY_DOMAIN_PACK` for AWC:
...
2. **Export your scenario values**, then launch with the bundled launcher (it creates
   ...
   ```bash
   export GLOSSARY_COMPANY="Arizona Water Company"
   export GLOSSARY_DOMAIN_PACK=domain_packs/water_utility.example.json
...
- **Database (live scan)** — PostgreSQL / MySQL / SQL Server, **read-only**. Reads
  schema, keys, and comments; sampling refines sensitivity and data quality. AWC's
  source schema is `awc_operations` (your real schema name; the app no longer assumes
...
(`/api/pdc/bulk-load/sample.csv`) or shipped as `datasources.sample.csv`. This set also
ships **`awc-datasources.csv`** — the same format pre-filled with the two AWC Workshop 1
connections, ready to load:

| kind | resourceName | reaches |
|---|---|---|
| `postgres` | `Arizona_Water_Operations` | `az-water-postgres:5432` · db `awc_operations` · user `pdc_user` · schema `public` |
| `minio` | `Arizona_Water_Documents` | `http://192.168.1.200:9000` · bucket `awc-documents` · path `/` |
